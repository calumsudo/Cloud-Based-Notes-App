...
export default function PersistentDrawerLeft({user, uid}) {
  ...
  const handleCreateNewNote = () => {
    const newNoteContent = ''; // Set the desired content for the new note
    saveNote(newNoteContent, uid);
    openNewNote();
  };

  const openNewNote = () => {
    const newNote = notes.find((note) => note.content === '');
  ...
  <Toolbar sx={{ justifyContent: 'space-between' }}>
    ...
    <ThemeProvider theme={theme2}>
      ...
      <Main open={open}>
        <DrawerHeader />
        {selectedNote && (
          <TextEditor uid={uid} note={selectedNote}/>
        )}
